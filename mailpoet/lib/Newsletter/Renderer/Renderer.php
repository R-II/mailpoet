...
class Renderer {
  ...
  private function _render(NewsletterEntity $newsletter, ?SendingQueueEntity $sendingQueue = null, $type = false, $preview = false, $subject = null) {
    ...
    if ($wpPost instanceof \WP_Post) {
      $this->wp->addFilter('woocommerce_email_editor_rendering_email_context', function($context) use ($newsletter, $sendingQueue): array {
        if ($sendingQueue) {
      ...
      $this->wp->removeAllFilters('woocommerce_email_editor_rendering_email_context');
    } else {
